...
class GUI:
  ...
  def makerandom(self):
    self.frame.setstructure([[random()>0.8 for x in range(self.width)] for y in range(self.height)])
    self.drawframe()
  def changesize(self):
  ...
  def drawframe(self):
    ...
    for y in range(self.height):
      for x in range(self.width):
        a=structure[y][x]
        if structure[y][x]:
          self.canvas.create_rectangle(x*(self.dispw/self.width),y*(self.disph/self.height),(x+1)*(self.dispw/self.width),(y+1)*(self.disph/self.height),width=0,fill="black")#*a+"white"*(not a))#"#"+"".join([str(hex(int(1024+random()*256)))[-2:] for i in range(3)]))
  def setseed(self):
...
class tgol_frame:
  def __init__(self,width,height,threads,ruleset):
    ...
    self.setstructure([[0 for z in range(width)] for i in range(height)])
  ...
  def setstructure(self,newstructure):
    ...
    #self.oldstructure=[[0 for z in range(self.width)] for i in range(self.height)]
  ...
  def clearframe(self):
    self.structure=[[0 for z in range(self.width)] for i in range(self.height)]
  def togglepoint(self,x,y):
    self.structure[y][x]=1-self.structure[y][x]
  ...
  def advance_frame(self,pool,chsize):
    ...
    self.structure=pool.map(self.isrowalive,range(self.height),chsize)
  ...
  def isalivenext(self,coords):
    ...
    if centre and sumn in self.survives:
      return 1#tframe[coords[1]][coords[0]]=1
    elif not centre and sumn in self.born:
      return 1#tframe[coords[1]][coords[0]]=1
    else:
      return 0#tframe[coords[1]][coords[0]]=0
  def neighbourvalue(self,coords):
    summ=0
    for i in self.tocheck:
      summ+=self.structure[(coords[0]+i[0])%self.height][(coords[1]+i[1])%self.width]
    return summ
#print(cpu_count())
